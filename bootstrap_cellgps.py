...
# 创建日志目录
log_dir = Path(os.path.dirname(sys.executable) if getattr(sys, 'frozen', False) else os.getcwd())
log_path = log_dir / "cellgps_startup.log"
...
logger = logging.getLogger('CellGPS_Startup')
...
{error_msg}")
    with open(log_dir / "cellgps_crash.log", 'a', encoding='utf-8') as f:
        f.write(f"\n[{datetime.now().isoformat()}] 程序崩溃\n")
        f.write(error_msg)
        f.write("\n---END OF CRASH---\n")
...
for name, module in sorted(sys.modules.items()):
    if module is not None:
        try:
            logger.debug(f"  {name}: {getattr(module, '__file__', '内置模块')}")
        except:
            logger.debug(f"  {name}: <获取路径时出错>")
